...
_DANGEROUS_PATTERNS = [
    (r"\bimport\s+os\b", "import os（可读取环境/文件系统）"),
    (r"\bfrom\s+os\b", "from os（可读取环境/文件系统）"),
    (r"\bimport\s+subprocess\b", "import subprocess（可执行任意系统命令）"),
    (r"\bfrom\s+subprocess\b", "from subprocess（可执行任意系统命令）"),
    (r"\bimport\s+importlib\b", "importlib（可动态导入绕过黑名单）"),
    (r"\bfrom\s+importlib\b", "importlib（可动态导入绕过黑名单）"),
    (r"\bimport\s+shutil\b", "shutil（可删除/移动文件）"),
    (r"\bimport\s+pathlib\b", "pathlib（可访问文件系统）"),
    (r"\bimport\s+getpass\b", "getpass（可读取密码）"),
    (r"\bimport\s+winreg\b", "winreg（可读写注册表）"),
    (r"\bimport\s+pickle\b", "pickle（反序列化可执行任意代码）"),
    (r"\bimport\s+pty\b", "pty（可打开伪终端）"),
    (r"\bimport\s+paramiko\b", "paramiko（SSH 客户端）"),
    (r"\bsubprocess\b", "subprocess（可执行任意系统命令）"),
    (r"\bos\.system\b", "os.system（可执行任意系统命令）"),
    (r"\bos\.popen\b", "os.popen（可执行任意系统命令）"),
    (r"\bos\.remove\b", "os.remove（可删除文件）"),
    (r"\bos\.unlink\b", "os.unlink（可删除文件）"),
    (r"\bos\.rmdir\b", "os.rmdir（可删除目录）"),
    (r"\bos\.environ\b", "os.environ（可读取环境变量）"),
    (r"\bos\.getenv\b", "os.getenv（可读取环境变量）"),
